...
# Line: 1D
# Tri/Tet : Good for arbitrary geometries
# Parallelogram/Parallelipiped : Good for large volumes and structured regions
# Prisms & Pyramids : Needed for bridging tet <-->Parallelipiped
...
class Edge:
    def __init__(
        ...
        self.verts = [
            Vertex(idx) for idx 
            in (start_idx + range(self.num_of_verts))
            ]
        self.cnctvty = np.array([
            vert.idx for vert in self.verts
            ])

class Face(ABC):
    def __init__(
        self,
        num_of_edges : NumericIntegerValueType,
        start_idx : IndexType
        ):
    ...
    def edges(self) -> Tuple[Edge, ...]:
        return tuple(
            [Edge((idx, idx+1)) for idx in range(self.num_of_edges)]
            )

    @property
    def cnctvty(self):
        return np.array([
            edge.cnctvty for edge in self.edges
            ])
